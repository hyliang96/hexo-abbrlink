...
let logic = function (data) {
    ...
    log.i('Full source [ %s ]', data.full_source);
    //+++
    if (data.layout == 'post') {
        let abbrlink;
        ...
        if (opt_AutoCategoryEnable && overwrite) {
            var old_categories = tmpPost.categories
            ...
            if (old_categories !== tmpPost.categories) {
                changed = true
            }
        }
        ...
        if (!abbrlink || abbrlink == '0' || config.force) {
            var old_abbrlink = tmpPost.abbrlink
            ...
            if (old_abbrlink !== tmpPost.abbrlink) {
                changed = true
            }
        }
